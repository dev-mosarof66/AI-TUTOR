...
const Button = ({ children, isNav = false }: ButtonProps) => {
  return (
    <div className={`${isNav ? "rounded-sm animate" : "w-full"}`}>
      <div
        className={`${
          isNav ? "px-2 py-1" : "w-[90%] xs:w-52 mx-auto sm:w-60  px-3 py-2  rounded-sm active:scale-95 "
